...
ann2012 <- read.csv('2012.annual.singlefile.csv', stringsAsFactors=F)
...
for(u in c('agglevel','area','industry',
           'ownership','size')){
  assign(u,as.data.table(read.csv(paste(u,'_titles.csv',sep=''), stringsAsFactors=F)))
}
...
names(county.fips)[1] <- "area_fips"
myarea <- merge(area, county.fips, by="area_fips",all.x=T, allow.cartesian = T)
myarea$state_fips <- substr(myarea$area_fips, 1,2)
names(mystatefips)[1] <- "state_fips"

myarea <- merge(myarea, mystatefips,by='state_fips', all.x=T, allow.cartesian = T)

ann2012full <- left_join(ann2012full, myarea)
ann2012full <- filter(ann2012full, state %in% lower48)

save(ann2012full, file='ann2014full.rda',compress=T)
...
chor <- chor[!is.na(chor$industry_code),]
ggplot(chor, aes(long,lat,group=group))+
...
unzip(file.path(zipfile), exdir='data') # unzips the file
csvfile <- gsub('zip','csv', zipfile) # Change file name
csvfile <- gsub('_','.',csvfile) # Change _ to . in name
dat <- fread(file.path(csvfile)) # read data
...
get_data <- function(zipfile){
 # unzip(zipfile, exdir='data') # unzips the file
  csvfile <- gsub('zip','csv', zipfile) # Change file name
  csvfile <- gsub('_','.',csvfile) # Change _ to . in name
...
files <- dir('data', pattern='annual.singlefile.zip') # file names
n <- length(files)
